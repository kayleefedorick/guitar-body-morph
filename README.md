...
**Generated bottom** - In Section Mix mode, replace the bottom path with one that is generated by creating a curved path that is G2 continuous on either side. This should also be constrained to the imaginary boundary box (Y-) around the guitar shape and tangent with it at one point. The bass and treble side paths can lengthened or shortened as needed in order to solve correctly for G2 continuous curvature.
