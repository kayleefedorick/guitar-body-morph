...
**Cutaway style transitions** - When morphing between a shape with a bass cutaway (e.g. Strat) and one without (e.g. Les Paul), the `inner_bass_cutaway` section degenerates because one guitar has no data for it. The current engine falls back to using the available section for both A and B, which produces a cutaway that does not fully disappear.
